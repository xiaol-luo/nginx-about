...
protected:
  ...
  /** Same as iter_forward_find_char(), but goes backward from @a iter.
...
public:
  ...
  /** Like iter_get_slice(), but invisible text is not included.
  ...
  /** Like iter_get_text(), but invisible text is not included.
   * Invisible text is usually invisible because a Gtk::TextTag with the
   * "invisible" attribute turned on has been applied to it.
   * @param end Iterator at end of range.
  ...
  /** This is equivalent to (iter_begins_tag() ||
   * iter_ends_tag()), i.e.\ it tells you whether a range with
  ...
  /** See iter_forward_cursor_position() or Pango::LogAttr or
  ...
  /** A convenience wrapper around iter_get_attributes(),
   ...
   * iter_forward_char() may actually move onto an image instead
   ...
   * the end iterator, and iter_forward_char() returns <tt>false</tt> for
   ...
   * offset 0), iter_backward_char() returns <tt>false</tt> for convenience when
   * writing loops.
  ...
  /** Calls iter_forward_word_end() up to @a count times.
   * @param count Number of times to move.
   * @return <tt>true</tt> if @a iter moved and is not the end iterator.
   */
  bool forward_word_ends(int count);
  
  /** Calls iter_backward_word_start() up to @a count times.
   * @param count Number of times to move.
   * @return <tt>true</tt> if @a iter moved and is not the end iterator.
   */
  ...
  /** Calls iter_forward_visible_word_end() up to @a count times.
   * @param count Number of times to move.
   * @return <tt>true</tt> if @a iter moved and is not the end iterator 
   * 
   * @newin{2,4}.
  ...
  /** Calls iter_backward_visible_word_start() up to @a count times.
   * @param count Number of times to move.
   * @return <tt>true</tt> if @a iter moved and is not the end iterator 
   * 
   * @newin{2,4}.
  ...
  /** Calls iter_forward_sentence_end() @a count times (or until
   * iter_forward_sentence_end() returns <tt>false</tt>). If @a count is
  ...
  /** Calls iter_backward_sentence_start() up to @a count times,
   ...
   * iter_backward_cursor_position() for details.
   * @return <tt>true</tt> if we moved and the new position is dereferenceable
   * 
   * @newin{2,4}.
   */
   ...
   * iter_set_line_index() if you have a byte index rather than
   * a character offset.
  ...
  /** Same as iter_set_line_offset(), but works with a
   ...
   * valid character in the buffer. iter_get_char() called on the
  ...
  /** Like iter_set_line_offset(), but the offset is in visible
  ...
  /** Like iter_set_line_index(), but the index is in visible
  ...
  /** Same as iter_forward_search(), but moves backward.
   ...
   * in those cases. There are some exceptions, such as iter_in_range(),
